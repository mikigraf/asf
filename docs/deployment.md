...
Evidence artifacts are the bytes an independent verifier re-reads, so production stores them on an S3-compatible endpoint rather than on local disk.

| Variable | Purpose |
| --- | --- |
| `ASF_ARTIFACT_S3_ENDPOINT` | HTTPS endpoint of the S3-compatible service; plaintext is refused outside loopback |
| `ASF_ARTIFACT_S3_REGION` | Signing region |
| `ASF_ARTIFACT_S3_BUCKET` | Bucket holding content-addressed artifacts |
| `ASF_ARTIFACT_S3_ACCESS_KEY_ID` | Access key identity |
| `ASF_ARTIFACT_S3_SECRET_ACCESS_KEY` | Secret key; held in memory as a secret and never logged or rendered |

All five are required together; a partial group is a startup error. Optional values are `ASF_ARTIFACT_S3_PATH_STYLE` (default `true`, which suits MinIO and most S3-compatible endpoints; set `false` for AWS virtual-hosted buckets), `ASF_ARTIFACT_S3_PREFIX` (default `sha256`), `ASF_ARTIFACT_S3_TIMEOUT_MILLISECONDS` (default 30000, bounded to 1ms..=300s), and `ASF_ARTIFACT_S3_ENCRYPTION` (`bucket-managed` by default, or `aes256`, or `aws-kms` with `ASF_ARTIFACT_S3_KMS_KEY_ID`).

Storage is content-addressed: one object per SHA-256 digest under the configured prefix, so writing the same bytes twice is the same object and a write is idempotent. Every request is signed with AWS Signature Version 4 over its own payload hash, so an intermediary cannot substitute bytes without invalidating the signature. Reads are re-digested before they are returned: an endpoint that answers with content that is not what was asked for produces an error rather than evidence. The configuration is validated at startup by constructing the store, so an unusable endpoint fails the daemon rather than the first verification.
